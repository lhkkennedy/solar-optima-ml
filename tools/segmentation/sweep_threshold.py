...
def sweep_threshold(
    ...
    with torch.inference_mode():
        for batch in val_loader:
            ...
            out = model(x)["out"]
            probs = torch.softmax(out, dim=1)[:, 1, :, :]  # (N,H,W)
            for t in thresholds:
                pred = (probs > t).to(torch.uint8)
                iou = compute_iou_binary(pred, y.to(torch.uint8))
                ious[t] += iou
            counts += 1
...
def parse_args():
    ap = argparse.ArgumentParser(description="Sweep threshold on validation set to maximize IoU")
